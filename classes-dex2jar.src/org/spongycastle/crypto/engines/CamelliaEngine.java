  ...
  implements BlockCipher
{
  ...
  private void ˊ(int[] paramArrayOfInt1, int[] paramArrayOfInt2, int paramInt)
  {
    ...
    i = aJv[(i & 0xFF)] ^ aJx[(i >>> 8 & 0xFF)] ^ aJw[(i >>> 16 & 0xFF)] ^ aJu[(i >>> 24 & 0xFF)];
    int j = paramArrayOfInt1[1] ^ paramArrayOfInt2[(paramInt + 1)];
    j = aJu[(j & 0xFF)] ^ aJv[(j >>> 8 & 0xFF)] ^ aJx[(j >>> 16 & 0xFF)] ^ aJw[(j >>> 24 & 0xFF)];
    paramArrayOfInt1[2] ^= i ^ j;
    paramArrayOfInt1[3] ^= i ^ j ^ ﹾ(i, 8);
    i = paramArrayOfInt1[2] ^ paramArrayOfInt2[(paramInt + 2)];
    i = aJv[(i & 0xFF)] ^ aJx[(i >>> 8 & 0xFF)] ^ aJw[(i >>> 16 & 0xFF)] ^ aJu[(i >>> 24 & 0xFF)];
    paramInt = paramArrayOfInt1[3] ^ paramArrayOfInt2[(paramInt + 3)];
    paramInt = aJu[(paramInt & 0xFF)] ^ aJv[(paramInt >>> 8 & 0xFF)] ^ aJx[(paramInt >>> 16 & 0xFF)] ^ aJw[(paramInt >>> 24 & 0xFF)];
    paramArrayOfInt1[0] ^= i ^ paramInt;
    paramArrayOfInt1[1] ^= i ^ paramInt ^ ﹾ(i, 8);
  }
 ...
 * Qualified Name:     org.spongycastle.crypto.engines.CamelliaEngine
